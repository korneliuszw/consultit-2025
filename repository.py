...
class TelemetryLogRepository:
    @staticmethod
    def get_in_month(session: Session, month: str) -> List[Type[TelemetryLogModel]]:
        """Month must be of format %m.%Y so 01.2025 => january of 2025"""
        return (
            session.query(TelemetryLogModel)
            .filter(func.strftime("%m.%Y", TelemetryLogModel.start_date) == month)
            .all()
        )
